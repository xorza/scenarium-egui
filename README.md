...
The egui app renders the sample graph via `src/gui/graph.rs`, which draws a transparent dotted background before delegating to `src/gui/node.rs` for node rectangles, labels, text, and cubic bezier connection curves. Rendering now shares a reusable `RenderContext` plus small widget renderers for background, connections, node bodies, ports, and labels to reduce duplication. Nodes can be dragged by their title bar to reposition them, and the graph view supports panning and zoom gestures stored on the graph itself (including scaled text and connection curvature). Node widths now auto-size to fit the widest label (node title, inputs, or outputs) while respecting the base minimum width, with no extra gap added between left and right label columns. Inputs and outputs are rendered with larger circular ports that stick out from the node sides; inputs are blue, outputs are blue-green, and each port brightens on hover. Each node now includes a small `x` button in the title bar for removing it (with hover/pressed feedback and a tooltip), which also clears any inbound connections. Panning triggers when you drag empty space with the primary button, drag anywhere with the middle mouse button, or scroll with a touchpad/scroll wheel when the cursor is over the graph. Pinch-to-zoom (trackpad) or Ctrl/Cmd + scroll zooms in/out (inverted direction, slightly faster), centered on the cursor. Clicking or dragging a node selects it and draws a brighter border using `Graph::selected_node_id`, while clicking empty background clears the selection. To break connections, click and drag on empty space to paint a red breaker stroke (up to a fixed length); intersected connections turn red and are removed when you release the mouse button. To create connections, click and drag from a port to a nearby port of the opposite type; releasing within the activation radius connects the ports and overwrites any existing input connection. Temporary connection drags now mirror the port direction, so starting from an input bends the curve left before heading to the target. The transient UI state for breakers/connection drags now lives in `gui::graph::GraphUi`, which `ScenariumApp` owns and calls to render the graph.
